...
if [ $1 = "-d" ]; then
	...
	gcc hello_glib.c `pkg-config --cflags --libs glib-2.0 gthread-2.0`  -o hello_release
elif [ $1 = "-r" ]; then
	...
	arm-linux-gnueabihf-gcc hello_glib.c `pkg-config --cflags --libs glib-2.0 gthread-2.0`  -o hello_release
fi
exit 0
